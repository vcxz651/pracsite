...
참조: `pracapp/models.py` (`Meeting.current_schedule_stage`)
...
- `BOOKING_IN_PROGRESS`
  - 조건: `is_schedule_coordinating=True`, `is_booking_in_progress=True`
  - 의미: 실제 합주실 예약 반영 단계
- `FINAL_SHARED`
...
- 예약 반영 단계 진입
  ...
  - 결과: `is_booking_in_progress=True`
- 최종 공유
  - 관련: `schedule_save_result` (final save)
  - 결과: `schedule_version += 1`, `is_final_schedule_released=True`, 조율/예약중 해제
- 최종 확정
  - 관련: `schedule_confirm`
  - 결과: `is_final_schedule_confirmed=True`, 필요 시 기타 플래그 정리
- 최종 리셋
