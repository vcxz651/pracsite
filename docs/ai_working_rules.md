...
- 이번 수정으로 막은 실패 경로(다시 하면 안 되는 구현 경로, 금지 패턴)
...
- 이번에 막은 버그가 "왜 생겼는지"가 다음 작업자의 구현 선택에 영향을 주면, 결과만 적지 말고 원인과 금지 경로도 함께 남긴다.
- 특히 `load_saved`, 임시 저장본, 상태 플래그 재해석처럼 겉보기 동작은 같아도 결과가 달라지는 우회 경로는 `금지 패턴`으로 명시한다.
...
8. 이번 작업에서 "다시 하면 안 되는 구현 경로"가 생겼으면, 관련 SSOT에 원인/금지 패턴까지 남겼는지 확인한다.
9. 다음 작업자에게 넘겨야 하면 `docs/session_handoff_template.md` 형식으로 세션 handoff를 남긴다.
