...
용도:
- 공개 시나리오 A와 레거시 내부 경로용 B/C 템플릿을 함께 미리 생성한다.
...
현재 운영 메모 (2026-02-28):
- 이번 배포 직전에는 기능 안정화와 데모 흐름 정합성을 우선한다.
- `match_result.html` 성능 최적화(hover 경량화, DOM 중복 축소, 전역 카드 구조 재검토)는 **이번 배포 범위에서 제외**하고, 배포 후 로컬 후속 작업으로 분리한다.
- `match_result.html` 스케줄 보드에서 요일 헤더 아래 방 이름 표시 행을 추가하는 UI 개선은 **이번 배포 범위에서 제외**하고, 배포 후 후속 구현 항목으로 미룬다.
- 실제 매치 결과 페이지 실사용 시점이 약 4주 남아 있으면, 운영 배포본을 먼저 안정화한 뒤 로컬에서 성능 개선을 충분히 검증하고 별도 재배포하는 것을 기본 계획으로 본다.
