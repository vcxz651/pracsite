# Meeting Handover (2026-02-27, Updated)
...
이 문서는 매칭/예약 도메인 상세 기준이므로, 프로젝트 전체 개요와 공통 가드레일을 먼저 맞춘 뒤 읽는다.
...
- 리모콘 필터(`강제 배치만`, `공유 후 변경만`, `임시합주실만`)는 관리자 hover-preview + click-pin 동작.
- 조율 보드는 현재 **룸슬롯 단일 모드**다. `일반/룸 보기` 전환 UI는 제거되었다.
- `저장 후 공유`, `임시 저장`, 예약 진입 버튼은 모두 현재 보드/예약 상태 기준으로 실시간 활성화된다.
- 자동 매칭의 인원/정원/가용시간 계산은 `needed`/`extra` 세션을 구분하지 않고 **모든 assignee**를 포함해 계산한다. 단, 같은 사람이 여러 세션에 중복 배정된 경우 인원 수는 1명으로만 센다.
- 매칭 설정 고급 옵션의 `주 2회 이상은 다른 날짜 강제`를 켜면, 같은 주차의 동일 곡은 같은 날짜에 중복 배치하지 않는다. 다른 날짜가 없으면 해당 주차는 실패 처리된다.
...
- `schedule_stage_code`, `schedule_stage_label`
  - 위 플래그 조합을 사람이 읽기 쉬운 상태 코드/라벨로 노출하는 모델 속성.
...
- 임시합주실 roomslot 기준 우측(최후순위) 정렬: `getRoomPriorityRank()`에서 `temp-*` 큰 rank 강제.
- 비관리자 리모콘에서 비활성 버튼 숨김.
- 관리자 리모콘 hover-preview + click-pin 동작 추가.
- 예약 단계의 `예약 임시 저장`은 카드 위치뿐 아니라 `booking_completed_keys` 변경에도 dirty-state가 반응한다.
- 예약 단계 상단 액션(`합주 재매칭`, `예약 임시 저장`, `예약 확정`, `Shift+클릭 안내`)은 관리자 예약 확정 배너 바로 아래로 이동했다.
...
- `meeting_detail` 현황판 인원 카드 색상 기준
  - 전체 곡 기준 이론 평균이 아니라, 실제 인원들의 배정 수 분포를 기준으로 본다.
  - 같은 악기 그룹 내부에서 실제 평균 배정 수를 기준점으로 삼고, 평균보다 낮을수록 `흰색`, 평균에 가까울수록 `초록`, 평균보다 높을수록 `빨강` 쪽으로 그라데이션 표시한다.
- `meeting_detail` 현황판 상단 `전 세션 배정 : N`
  - 배정 요약 카드들과 같은 상태 필터 축으로 동작한다.
  - 배정 요약 카드는 상호배타 1개만 활성화되고, 인원 카드 다중 선택과는 동시에 함께 적용된다.
  - `전 세션 배정`과 `전 세션 지원`은 의미상 겹칠 수 있어도, 현황판 집계/필터 버킷은 기능적으로 분리한다.
  - 즉, `전 세션 배정`에 잡힌 곡은 `전 세션 지원` 요약 카드에 중복 집계/노출하지 않고 `전 세션 배정` 버킷을 우선 적용한다.
  - 리모콘 안 `전 세션 배정` 요약 카드는 진초록 바탕 + 흰 글자를 사용한다.
  - 실제 곡 카드는 카드 전체 대신 왼쪽 번호 배지 칸만 진초록으로 강조한다.
  - sticky 패널 상단 제어 순서는 `배정 리모콘 -> 검색 -> 지원자 전체 펼치기/접기 -> 요약 카드 구역 -> 안내 텍스트` 기준으로 유지한다.
...
- roomslot 기준에서 임시합주실은 최후순위(오른쪽) 배치.
...
## 14) 작업 시 절대 규칙
- `match_result.html`은 10k+ 라인이라 작은 수정도 부작용 큼. 수정 전/후 핸들러 중복 바인딩 반드시 확인.
...
2. `match_result.html` 성능 정리
- hover 경로 DOM 탐색 캐싱, 중복 DOM 축소, 주차 보드 렌더 부담 완화부터 우선한다.
- 새 전역 곡 카드 시스템은 구조 리스크가 높으므로, 이번 배포 직전이 아니라 로컬 후속 작업으로 검증 후 별도 배포한다.
...
추가 로드맵 메모:
- 배포 후 바로 할 일: 대형 소스파일 리팩토링
- 중장기 목표: 커뮤니티 기능, `연합공연` 기능, 합주실 독립모델 본격 가동
- 장기 목표: 실제 합주실 페이지와 연동
...
- **[P1] 임시합주실 `get_or_create` 이름 충돌 덮어씀 위험**: `matching_views.py` 약 2510줄 부근에서 `PracticeRoom.get_or_create(band=..., name=..., location=...)` 시 기존 영구 합주실과 이름/위치가 동일하면 `is_temporary=True`로 강제 업데이트될 수 있음. 실 운영 합주실이 임시합주실로 오염되는 시나리오 → 수정 전까지 `get_or_create` 대신 명시적 `create` + 고유 prefix 사용 검토 필요.
- **[P2] `disableAutoTempRoomReassign` dead const**: `match_result.html` L2440에 선언된 상수이나 코드 내 어디서도 참조되지 않음. 기능 미완성 흔적으로 추정. 향후 정리 또는 실제 로직 구현 여부 결정 필요.
